...
const LegendListInner = typedForwardRef(function LegendListInner<T>(
  ...
  const getId = (index: number): string => {
    ...
    const ret =
      index < data.length ? (keyExtractor ? keyExtractor(data[index], index) : index) : null;
    return `${ret}`;
  };
  ...
  const getItemSize = (key: string, index: number, data: T) => {
    ...
    const size =
      (getEstimatedItemSize ? getEstimatedItemSize(index, data) : estimatedItemSize) ??
      DEFAULT_ITEM_SIZE;
  ...
  const addTotalSize = useCallback(
    (key: string | null, add: number, totalSizeBelowAnchor: number) => {
      const state = refState.current!;
  ...
  const calculateItemsInView = useCallback((speed: number) => {
    ...
    const topPad =
      (peek$<number>(ctx, 'stylePaddingTop') || 0) + (peek$<number>(ctx, 'headerSize') || 0);
    ...
    let loopStart: number = startBufferedIdOrig
      ? state.indexByKey.get(startBufferedIdOrig) || 0
      : 0;
    ...
    const nextTop = Math.ceil(
      startBuffered !== null ? positions.get(startBufferedId!)! + scrollBuffer : 0
    );
    const nextBottom = Math.floor(
      endBuffered !== null
        ? (positions.get(getId(endBuffered! + 1))! || 0) - scrollLength - scrollBuffer
        : 0
    );
  ...
  if (
    isFirst ||
    dataProp !== refState.current.data ||
    numColumnsProp !== peek$<number>(ctx, 'numColumns')
  ) {
  ...
  const memoizedLastItemKeys = useMemo(() => {
    ...
    return new Set(
      Array.from({ length: Math.min(numColumnsProp, dataProp.length) }, (_, i) =>
  ...
  const stylePaddingTop =
    ...
    StyleSheet.flatten(contentContainerStyle)?.paddingTop ??
    0;
  ...
  const doInitialAllocateContainers = () => {
    ...
    if (scrollLength > 0 && !peek$(ctx, 'numContainers')) {
      const averageItemSize =
        estimatedItemSize ?? getEstimatedItemSize?.(0, dataProp[0]) ?? DEFAULT_ITEM_SIZE;
      const numContainers =
        Math.ceil((scrollLength + scrollBuffer * 2) / averageItemSize) * numColumnsProp;
